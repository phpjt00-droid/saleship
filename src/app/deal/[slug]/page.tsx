...
export default function DealDetailPage() {
  ...
  useEffect(() => {
    const fetchDeal = async () => {
      try {
        const data = await dealService.getDealBySlug(slug)
        setDeal(data)
        
        // 페이지 로드 시 조회수 증가
        if (data.id) {
          dealService.incrementViewCount(data.id)
        }
      } catch (error) {
  ...
  if (loading) {
    return (
      <div className="container py-24 animate-pulse">
        <div className="h-10 bg-slate-200 dark:bg-slate-800 w-3/4 mb-6 rounded-lg"></div>
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
          <div className="aspect-square bg-slate-200 dark:bg-slate-800 rounded-3xl"></div>
          <div className="space-y-6">
            <div className="h-32 bg-slate-200 dark:bg-slate-800 rounded-lg"></div>
            <div className="h-10 bg-slate-200 dark:bg-slate-800 w-1/2 rounded-lg"></div>
          </div>
        </div>
      </div>
    )
